...
function PricingCard({
  ...
  return (
    <article
      className={`rounded-[28px] border px-5 py-6 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.015)] md:px-6 ${
        highlighted
    ...
    >
      <h3 className="font-sora text-[24px] font-semibold leading-none text-white md:text-[30px]">
        {name}
      </h3>

      <p className="mt-4 max-w-68 text-[15px] leading-[1.35] text-[#c4bdd4]">
        {description}
      </p>

      <div className="mt-6 flex items-start text-white">
        <span className="mt-2 text-[24px] leading-none">$</span>
        <span className="ml-1 font-sora text-[58px] font-semibold leading-[0.9] tracking-[-0.05em] md:text-[66px]">
          {price}
        </span>
      </div>

      <button
        className={`mt-6 flex w-full items-center justify-center rounded-xl border px-5 py-2.5 text-[15px] font-medium text-white transition ${
          highlighted
      ...
      <ul className="mt-8 space-y-0">
        {features.map((feature, index) => (
          <li
            key={feature}
            className={`${index !== 0 ? "border-t border-white/18" : ""} flex items-center gap-3 py-4`}
          >
            <span className="h-4 w-4 shrink-0 rounded-full bg-[#6a3cff] shadow-[0_0_16px_rgba(109,68,255,0.4)]" />
            <span className="text-[15px] text-[#d3cedf]">{feature}</span>
          </li>
        ))}
      </ul>
    </article>
    ...
    <section id="pricing" className="py-16 md:py-20">
      <div className="mx-auto max-w-310">
        <div className="text-center">
          ...
          <h2 className="mt-6 font-sora text-[36px] font-semibold leading-[1.03] tracking-[-0.05em] text-[#dcd9eb] md:text-[48px]">
            Our{" "}
